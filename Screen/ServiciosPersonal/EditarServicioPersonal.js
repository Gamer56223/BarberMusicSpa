...
export default function EditarServicioPersonal({ route, navigation }) {
    ...
    useEffect(() => {
        if (serviciopersonalId) {
            const foundServicioPersonal = serviciopersonalesEjemplo.find(sp => sp.id === serviciopersonalId);
            if (foundServicioPersonal) {
                setCategoria(foundServicioPersonal);
                setId(foundServicioPersonal.id);
                setServicio_Id(foundServicioPersonal.Servicio_Id);
                setPersonal_Id(foundServicioPersonal.Personal_Id);
    ...
    const handleSave = () => {
        const serviciopersonalData = {
            id: serviciopersonalId || new Date().getTime().toString(), // Generar ID si es nueva
            Servicio_Id: servicio_id,
            Personal_Id: personal_id,
        };
        console.log("Datos a guardar:", serviciopersonalData);
    ...
    if (loading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>Cargando datos del Servicio Personal...</Text>
            </View>
        );
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            <Text style={styles.title}>{serviciopersonalId ? "Editar servicio personal" : "Crear Nuevo servicio personal"}</Text>

            <ScrollView style={styles.formScrollView}>
                <View style={styles.formCard}>
                    <Text style={styles.label}>Nombre del servicio personal:</Text>
                    <TextInput
                        style={styles.input}
                        value={id}
                        onChangeText={setId}
